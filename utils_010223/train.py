...
parser.add_argument("--multi",          help = "if True : multiple output (multiple heads) ",                 action = "store_true"                                                         )
...
parser.add_argument("--train_reverse",  help = "0: no revcomp., 1: revcomp. only, 2: both ",                  type = int,     default = 0,              required=False, choices=[0,1,2]     )
parser.add_argument("--train_weights",  help = "0: None, 1: array, 2: dict, 3: undersampling",                type = int,     default = 1,              required=False, choices=[0,1,2,3,5] )
...
parser.add_argument("--val_reverse",    help = "0: no revcomp., 1: revcomp.  only, 2: both",                  type = int,     default = 0,              required=False, choices=[0,1,2]     )
parser.add_argument("--val_weights",    help = "0: No weights, 1: array , 2: dict, 3: undersampling",         type = int,     default = 0,              required=False, choices=[0,1,2,3]   )
...
with strategy.scope():
    if model_name == "MNase_simple" and multi : model_name = "CNN_simple5H"
...
if multi:
    ...
    train_gen = DNAmulti5H(seq=[seq.format(x) for x in train_chr],
                    ...
                    zeros=train_zeros, winsize=winsize, frac=train_frac, sample=train_sample, apply_weights=1, reverse=0, N=train_N)
