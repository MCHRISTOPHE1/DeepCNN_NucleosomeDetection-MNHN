...
parser.add_argument("--offset",     help = 'start of the mutasome (to align with sequence)', default = 3_000_000, type=int, required = False)
parser.add_argument("--thresh",    help = "#Mutasome threshold to select peaks", default = 10, type = float, required = False)
parser.add_argument("--gap",  help = "#Max peak distance to Nucleosome Positionning Region",default = 20, type = int, required=False)
parser.add_argument("--size",help = "#half size of sequence output",  type = int,     default=10,   required=False)

parser.add_argument("--output", help = "output directory path",   type=str, required=True)
parser.add_argument("--mutasome", help = "mutasome path",type=str,required = True)
parser.add_argument("--repeat",      help = "repeat masker formatted file path", type = str,   required=True)
...
for chr in range(1,20):
    ...
    f_inter_pos = np.array([x[len(x)//2] for x in inter_pos])
