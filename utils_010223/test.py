...
parser.add_argument("--path",     help= "user specified path to store results", default=None, required=False, type = str)
parser.add_argument("--comments", help = "comment to add in the readme", default = "", required=False, type = str)
...
if type(args.path) != type(None):
        output_path = args.path
else:
        output_path = global_path + "/predictions"
...
if multi>0:
        ...
        if multi>1:
                try:
                        np.save(output_path + "/RAWRESULT_{}__".format(chr) + args.model, model_result)
                        print("Raw pred saved at {}".format(global_path+"predictions/RAWRESULT_{}__".format(chr) + args.model))
                except FileNotFoundError:
                        np.save("RAWRESULT_{}__".format(chr) + args.model, model_result)
                        print("!!!!! NO DIRECTORY, FILE SAVED IN CURRENT DIRECTORy !!!!!!")
                        print("Raw pred saved at {}".format(global_path+"predictions/RAWRESULT_{}__".format(chr) + args.model))
        pred = np.zeros(lseq)
        ...
        try:
                np.save(output_path + "/{}__".format(chr) + args.model, pred)
        except FileNotFoundError:
                np.save("{}__".format(chr) + args.model, pred)
                print("!!!!! NO DIRECTORY, FILE SAVED IN CURRENT DIRECTORy !!!!!!")
                print("Raw pred saved at {}".format(global_path+"predictions/RAWRESULT_{}__".format(chr) + args.model))

print("saved at {}".format(output_path + "/{}__".format(chr) + args.model))
