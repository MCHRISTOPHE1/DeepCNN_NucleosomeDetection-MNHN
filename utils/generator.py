                ...
                for i,idx in enumerate(image_idx):
                    ...
                    if self.reverse==1:
                        try:
                            rdx = reverse_idx[i]
                            im2 = self.rseq[int(rdx)-(self.winsize//2):int(rdx+self.winsize//2)+1,:].reshape((self.winsize,4))[::-1,:]
                            class2 = self.lab[[rdx - (self.winsize//2) + x for x in step][::-1]]
                        except ValueError:
                            continue
                        classes.extend([classe, class2])
                        images.extend([im,im2])
...
class DNA_train():
    ...
    def __init__(self,seq,frac=1, winsize = 2001, reverse=0):
        """
        :param str seq: path to a numpy array stored as npz in ["arr_0"], the array contains the one-hot encoded DNA/RNA sequence
        :param str lab:  path to a numpy array stored as npz in ["arr_0"], the array contains labels for the center of the considered window
        :param str weights: path to a numpy array stored as npz in ["arr_0"], the array contains weights for each label
        ...
        if seq[-1] == "y":
            self.seq = np.load(seq).reshape((-1,4))            #Sequence is truncated to fit the labels
        elif seq[-1] == "z":
            self.seq = np.load(seq)["arr_0"].reshape((-1,4))
        else:
            raise ValueError("file: '{}' is nor .npz nor .npy".format(seq))
            ...
            for i,idx in enumerate(image_idx):
                try:
                    if self.reverse == 0:
                        ...
                        im = self.seq[int(idx)-self.winsize//2:int(idx+self.winsize//2)+1,::-1].reshape((self.winsize,4))
                    images.append(im)
                except ValueError:
                    continue
                # yielding condition
        ...
        if self.apply_weight==2: #Weights dict
            ...
            while True:
                for i,idx in enumerate(image_idx):
                    ...
                    try:
                        im = self.seq[int(idx)-(self.winsize//2):int(idx+(self.winsize//2))+1,:].reshape((self.winsize,4))
                        classe = self.lab[idx-500:idx+501]
                    except ValueError:
                        continue
            ...
            while True:
                for i,idx in enumerate(image_idx):
                    try:
                        im = self.seq[int(idx)-self.winsize//2:int(idx+self.winsize//2)+1,:].reshape((self.winsize,4))
                        classe = self.lab[idx-500:idx+500]
                    except ValueError:
                        continue
